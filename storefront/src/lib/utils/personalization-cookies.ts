...
const SEARCH_HISTORY_PRODUCT_IDS_COOKIE = `${COOKIE_PREFIX}search_history_product_ids`
...
/**
 * Adds a product ID to the search history array (max 10, FIFO)
 */
export function addSearchHistoryProduct(productId: string) {
  const existing = getCookie(SEARCH_HISTORY_PRODUCT_IDS_COOKIE)
  let products: string[] = []
  ...
  setCookie(SEARCH_HISTORY_PRODUCT_IDS_COOKIE, products.join(","))
}
